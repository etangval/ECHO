# Digital Creativity: Event-Preserving Sonification
...
This is the public implementation accompanying **“Event Preserving Sonification through Higher Order Harmonic Optimisation”** (manuscript in preparation). The repository contains **code and artificial examples only**. Original neural, earthquake and X-ray records, prepared observations, study responses, private mappings and recorded study stimuli are not included.
...
```sh
git clone https://github.com/etangval/digitalcreativity.git
cd digitalcreativity
python -m venv .venv
```
...
The random control permutes the optimised pitch inventory while keeping event times, velocities and category-specific stereo positions fixed. This is a particular controlled baseline, not the only definition of random sonification; report the baseline and seeds used in any study.
...
```sh
python -m pip install ".[audio,fits,test]"
python -m pytest -q
```
...
Use the [v0.1.0 release](https://github.com/etangval/digitalcreativity/releases/tag/v0.1.0) for a fixed code snapshot. Record the release/commit, installed dependency versions and generated manifest in your own work. A seed alone does not promise identical floating-point results across every numerical-library version or platform.
...
元の研究データ・調査回答・実データ由来の中間ファイルは公開していません。再構成できるのは、保持したカテゴリの準備済みイベントと時刻です。除外した観測や連続値の属性、MP3から元データが復元できるという意味ではありません。音の美しさや好感度の向上は、このコードだけで実証されるものではありません。
