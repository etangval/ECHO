...
The [30-minute sheep piano study](https://www.youtube.com/watch?v=1fWy2ZjXSEk) uses this within-step timing model with 50 identities, 3,602 events, and uniform five-fold time compression: 120.1 note attacks/minute over the complete recording. Five-minute segments contain 49–50 active pitches and average 98.6–134.4 attacks/minute. The final ten seconds contain no new note attacks.

This timing revision retains the endpoint edition's selected events, optimised pitch map, per-event velocities and stereo positions. It does not rerun pitch optimisation on the interpolated times. The observed-endpoint selection spans 14 December 2018, 02:03:54–04:32:59 UTC; its first inferred crossing may precede that selection origin by less than one six-second step. A 1.2-second playback lead accommodates this advance. Source files remain unchanged.
